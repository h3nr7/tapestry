package uk.co.tapestry.view {
	...
	public class FilmsMediator extends SimpleMediator implements IMediator {
		...
		public function Init():void {
			...
			viewComponent		= new Films( ((facade as ApplicationFacade).getContainerByName('mainContainer') as Sprite), dD.getNewsList());
			(component as Films).addEventListener(StateChangeEvent.ANIMATION_IN_COMPLETE, onAnimationInComplete);
			(component as Films).addEventListener(StateChangeEvent.ANIMATION_OUT_COMPLETE, onAnimationOutComplete);	
			trace('News Mediator Init');
		}
		
		public function Kill():void {
			(component as Films).Kill();
		}
